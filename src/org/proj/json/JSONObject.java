...
public class JSONObject {
    ...
    public ReadOnlySet<Map.Entry<String,Object>> entrySet () {
        return ReadOnlyMap.ofMap(this.data).entrySet();
    }
    ...
    public <T> T get (String name, JSONDecoder<T> decoder) {
        return decoder.decode(getObject(name));
    }
    ...
    public String[] getStringArray (String name) {
        ...
        return wrapper.stream().map(Object::toString).toArray(String[]::new);
    }
    ...
    public <T> T[] getArray(String name, JSONDecoder<T> decoder) {
        JSONObject[] array = getObjectArray(name);
        T[] result = (T[]) Array.newInstance(decoder.getTransformClass(), array.length);

        Range.ofInt(0, array.length, true).forEach(i -> {
            JSONObject json = array[i];
            T element = decoder.decode(json);
            result[i] = element;
        });
    ...
    public <T> JSONObject put (String name, JSONEncoder<T> encoder, T value) {
        return put(name, encoder.encode(value));
    }
    ...
    public <T> JSONObject put (String name, JSONEncoder<T> encoder, T... value) {
        return put(name, Arrays.stream(value).parallel().map(encoder::encode).toArray(JSONObject[]::new));
    }
