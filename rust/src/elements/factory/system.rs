use std::{time::Duration, intrinsics::transmute, cmp::Ordering, rc::Rc, sync::Mutex, ops::DerefMut};

use crate::{Durationx};
use llml::{vec::EucVecd2, others::Complxd};
use rand::{prelude::{Distribution, ThreadRng}, distributions::{Standard, Uniform}, random, thread_rng, Rng};
...
async fn create_system () -> PlanetSystem {
    ...
    let star : Star = Gaussian::new().sample(&mut thread_rng);
...
impl GeneticSystem {
    ...
    fn new (system: PlanetSystem, len: usize, mutation_rate: f64, mut rng: ThreadRng) -> Self {
        ...
        Self { systems, mutation_rate, rng: Rc::new(Mutex::new(rng)) }
    }
    ...
    async fn epoch (&mut self, selection_size: Option<usize>, time_step: Duration) -> Result<(), JoinError> {
        ...
        let new_systems = self.systems.iter_mut().for_each(|system| {
            ...
            let mut lock = self.rng.lock().unwrap();
            let ((father, father_weight), (mother, mother_weight)) = Self::selection_parents(&mut lock, selection, sum);
            
            let weight_sum = *father_weight + *mother_weight;
            let limit = *father_weight / weight_sum;

            for planet in system.planets.iter_mut() {
                planet.velocity = if Standard.sample::<f64>(lock.deref_mut()) <= limit { father }
            }
    ...
    fn selection_parents<'a> (rng: &'a mut ThreadRng, values: &'a [(PlanetSystem, f64)], max: f64) ->(&'a (PlanetSystem, f64), &'a (PlanetSystem, f64)) {
