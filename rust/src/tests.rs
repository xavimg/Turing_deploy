...
pub async fn simulate_system () {
    ...
    let max_dist = system.planets.last().unwrap().position.norm() * 1.5;
    ...
    for planet in system.planets.iter() {
        let circle = window.create_circle((planet.position / max_dist).into(), planet.radius as f32, Color::RED.into()).unwrap();
        planets.push(circle);
    }
