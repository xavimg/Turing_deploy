...
pub fn simulate_system () {
    ...
    let max_dist = system.get_planets()[0].position.norm() * 2.;
    let ogl = Arc::new(OpenGl::new().unwrap());
    
    let window = ogl.create_instance("Testing planet factory", 900u32, 900u32).unwrap();
    let mut window = window.write().unwrap();

    let _ = window.create_circle(EucVecf2::default(), 0.25, Color::WHITE.into()).unwrap();
    ...
    thread::spawn(move || {
        loop {
            ...
            lock.simulate(Duration::from_months(3u32));
            planet_circle.write().unwrap().position = (lock.get_planets()[0].position / max_dist).into();
